...
def t_no_repair_semantics_and_source():
    ...
    sl_seg = '\n'.join(lines[4060:4110])  # F2 SL terminal 分支（2026-08-21 17:2x 实测）
    tp_seg = '\n'.join(lines[4195:4245])  # F2 TP terminal 分支
    ...
    #     即 F2 分支 user_modified → current_sl_id=None 但无 need_recover（修改后实测 L4082-4084 区）
    seg_user = '\n'.join(lines[4078:4090])
