...
class _Holder:
    # ══════════════════════════════════════════════════════════════════
    # 2026-08-29 -4061 事故 · v5（ChatGPT 对 v4 的终审：架构批准、diff 不批准）
    #
    ...
    def _begin_close_request_if_active(self, symbol: str, batch_id: str,
        ...
        返回 (ok: bool, close_op_id: str, reason: str)。
          ok=False → 调用方**立即返回，绝不发出任何交易所订单**。
        ...
        """
        if not close_reason:
            return False, '', 'missing_close_reason（BEGIN 必须带分型原因）'
        with self._state_lock:
            ...
            if int(b.get('close_phase', 0) or 0) != 0:
                return False, '', (f'close_phase_not_zero（disk='
                                   f'{b.get("close_phase")}，已有平仓事务在途）')
            if b.get('pending_close'):
                return False, '', 'pending_close_already_set'
            if b.get('settled_by_limit_close'):
                return False, '', 'settled_fact_present（结算事实已发生，绝不重启）'
