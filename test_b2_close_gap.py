...
# ⚠️ 第二轮审查（2026-08-21 17:2x）：update_batch_tp 清 invalid flag(+3) → 用户改SL/保本损 +3，部分减仓 +3；
#    F2 SL/TP fallback reason(+11) 在 3820 之后 → 部分减仓/部分减仓TP 再 +11 → 用户改TP→1322 | 用户改SL→1476 |
#    保本损→1690 | 部分减仓SL→3820 | 部分减仓TP→3894（Grep 实测，AST 函数归属核实）
# ⚠️ GLM 审计修复（2026-08-21 19:3x）：漏洞4 update_batch_sl 0值(+3) → 1476/1690/3820/3894 各 +3；
#    漏洞3(+12)+漏洞4(+6) 在 3820 之前 → 部分减仓/TP 再 +18 → 1322→1322 | 1476→1479 | 1690→1693 | 3820→3843 | 3894→3917
# GAP_CREATE_LINES = {1322, 1479, 1693, 3843, 3917}  # GLM审计修复后重新 Grep 实测（2026-08-21，AST 函数归属核实）
# ⚠️ D-006 账户层风控（2026-08-28）插码 +116 后未同步（HEAD 实测零交集，G9 陈旧 FAIL 基线）
# ⚠️ D-010 Batch 2（2026-08-28）插码 +235（前4处 +229）→ AST 重新实测并同步：
#    用户改TP→1651 | 用户改SL→1808 | 保本损→2022 | 部分减仓SL→4194 | 部分减仓TP→4268
GAP_CREATE_LINES = {1651, 1808, 2022, 4194, 4268}  # D-010 Batch2 后 AST 实测（2026-08-28，函数归属核实）
