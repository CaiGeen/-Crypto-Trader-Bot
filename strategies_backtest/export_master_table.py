"""合并所有回测结果为一张总表CSV，可在Excel自由筛选透视。

每个止盈配置(纯KAMA/12-18%)×每个波段一行，含全部字段。
"""
import os
import sys
...
def simulate(signal, tp=0.12):
    ...
    mfe_r = (max_exc * bt) / av if max_exc != -np.inf else 0.0
    # 两种偏离度
    ...
    return {
        ...
        "实际涨跌幅(%)": round(ret * 100, 2),
        "R值": round(r, 2),
        "盘中最大涨跌幅MFE(%)": round(float(max_exc) * 100, 2) if max_exc != -np.inf else 0.0,
        "MFE的R值": round(mfe_r, 2),
        "MFE是否≥10%": "是" if abs(max_exc) >= 0.10 else "否",
        "MFE是否≥8%": "是" if abs(max_exc) >= 0.08 else "否",
        "持仓K线数": exit_idx - i,
        "持仓小时": (exit_idx - i) * 4,
    }
...
for tp in tp_configs:
    ...
    n_ge10 = sum(1 for r in results if r["MFE是否≥10%"] == "是")
    print(f"止盈{label}: {len(results)}波段，≥10%有{n_ge10}单")

total_df = pd.DataFrame(all_rows)
out_path = cfg.base_dir / "results" / "回测总表.csv"
total_df.to_csv(out_path, index=False, encoding="utf-8-sig")
print(f"\n总表已保存: strategies_backtest/results/回测总表.csv")
print(f"总行数: {len(total_df)}，列数: {len(total_df.columns)}")
print(f"\n列名: {list(total_df.columns)}")
print(f"\n止盈配置分布:")
print(total_df["止盈配置"].value_counts().to_string())
print(f"\n≥10%波段数: {(total_df['MFE是否≥10%']=='是').sum()}")
