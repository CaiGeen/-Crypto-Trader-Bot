...
print("=" * 100)
print("新口径下：偏离度区间表现测试（区分多空方向）")
print("=" * 100)

def detect_all_swings(min_kama_run=20):
    swings = []
    i = 1
    while i < n:
        ...
        # 做空：KAMA掉头下降
            ...
            if kama_run >= min_kama_run:
                amp = (start_price - low) / start_price * 100 if start_price > 0 else 0
                duration = end_idx - turn_idx
...
swings = detect_all_swings(min_kama_run=20)
swings_df = pd.DataFrame(swings)

print(f"\nKAMA同向>=20K的大波段数: {len(swings_df)}")
print(f"做多: {(swings_df['direction']=='做多').sum()}个")
print(f"做空: {(swings_df['direction']=='做空').sum()}个")
...
long_df = swings_df[swings_df['direction'] == '做多']
print(f"\n做多波段数: {len(long_df)}")
print(f"涨跌幅中位: {long_df['amp'].median():.2f}%")
...
short_df = swings_df[swings_df['direction'] == '做空']
print(f"\n做空波段数: {len(short_df)}")
print(f"涨跌幅中位: {short_df['amp'].median():.2f}%")
