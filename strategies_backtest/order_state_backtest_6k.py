...
machines = {"long": {"order": None, "positions": []},
            "short": {"order": None, "positions": []}}
bands = []      # 全波段记录（含未成交）
trades = []     # 成交明细
...
for t in range(6, n):
    for d, mc in machines.items():
        ...
        if o is not None:
            ...
            if t >= 2:
                if is_long:
                    new_swing = highs[t-2] > o["base"] and highs[t-1] <= highs[t-2] and highs[t] <= highs[t-2]
                else:
                    new_swing = lows[t-2] < o["base"] and lows[t-1] >= lows[t-2] and lows[t] >= lows[t-2]
                if new_swing:
            ...
            if o is None:
                ...
                if o is not None:
                    seg_h = highs[o["base_idx"]:t+1].max(); seg_l = lows[o["base_idx"]:t+1].min()
                    if (seg_h - seg_l) > 3 * atr[t]:
                        o["end_t"] = t; o["status"] = "3ATR区间撤单"
                        bands.append(o); mc["order"] = None
        else:
            # IDLE：新武装（任意2K确认的摆动高点/低点）
            if t >= 2:
                if is_long:
                ...
                if swing:
                    if is_long:
                        dominant = highs[t-2] >= highs[max(0,t-7):t].max()
                    else:
                        dominant = lows[t-2] <= lows[max(0,t-7):t].min()
                    if dominant:
                        mc["order"] = new_order(d, t-2, t)
...
full.to_csv("results/全波段明细_订单状态机_6K门槛.csv", index=False, encoding="utf-8-sig")
trade_df.to_csv("results/成交明细_订单状态机_6K门槛.csv", index=False, encoding="utf-8-sig")
