            ...
            if bh <= 3:
                ...
                if bh == 1:
                    ...
                    if (closes[t] < conf) if is_long else (closes[t] > conf):
                        close_position(p, t, float(closes[t]), "保护性平仓", "第1K"); continue
...
full.to_csv("results/全波段明细_订单状态机_仅第1K保护.csv", index=False, encoding="utf-8-sig")
trade_df.to_csv("results/成交明细_订单状态机_仅第1K保护.csv", index=False, encoding="utf-8-sig")
