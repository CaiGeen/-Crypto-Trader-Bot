...
print("=" * 105)
print("止盈 12%~18% 对比 — EMA70偏离度过滤 [-5%, 15%)")
print("=" * 105)

for label, use_filter in [("无过滤（基准）", False), ("EMA70偏离度[-5%,15%)", True)]:
    print(f"\n{'─'*105}")
    print(f"  {label}")
    print(f"{'─'*105}")
    ...
    for tp in [0.12, 0.13, 0.14, 0.15, 0.16, 0.17, 0.18]:
        ...
        for d, lbl in [("long", "做多"), ("short", "做空")]:
            sub = f[f["direction"] == d]
            if sub.empty:
                print(f"{tp:>6.0%} {lbl:>6} {0:>6} {'N/A':>8} {'N/A':>8} {'N/A':>8} {'N/A':>8} {'N/A':>10} {'N/A':>8}")
                continue
            ...
            print(f"{tp:>6.0%} {lbl:>6} {len(sub):>6} {sr.sum():>8.1f} {sr.mean():>8.2f} {sr.median():>8.2f} "
