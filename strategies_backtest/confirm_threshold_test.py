...
def simulate(signal, use_confirm=True):
    i = signal.idx
...
def run_backtest(use_confirm):
    results = []; last_exit = -1
    for s in signals:
        if s.idx <= last_exit: continue
        r = simulate(s, use_confirm)
        if r and passes(r):
            results.append(r)
            last_exit = r["exit_idx"]
    return pd.DataFrame(results)

rdf_confirm = run_backtest(True)
rdf_no = run_backtest(False)
...
D = f"{'━'*95}"
print("=" * 95)
print("确认阈值测试：有确认(0.5ATR) vs 无确认(全部持仓)")
print("=" * 95)

print(f"\n{D}\n一、回测结果对比\n{D}")
print(f"  有确认:  {stats(rdf_confirm)}  回撤={max_dd(rdf_confirm):.1f}R")
print(f"  无确认:  {stats(rdf_no)}  回撤={max_dd(rdf_no):.1f}R")

print(f"\n{D}\n二、出场原因对比\n{D}")
for mode, rdf in [("有确认", rdf_confirm), ("无确认", rdf_no)]:
    print(f"  {mode}:")
...
weighted_eval(rdf_confirm, e_prime, "E' × 有确认")
weighted_eval(rdf_no, e_prime, "E' × 无确认")

print(f"\n{D}\n四、逐年对比\n{D}")
print(f"  {'年份':>6} {'有确认':>30} {'无确认':>30}")
for yr in sorted(rdf_confirm["year"].unique()):
    s1 = rdf_confirm[rdf_confirm["year"]==yr]
    s2 = rdf_no[rdf_no["year"]==yr]
