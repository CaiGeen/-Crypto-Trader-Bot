    ...
    for d, mc in machines.items():
        ...
        for p in list(mc["positions"]):
            ...
            if bh <= 3:
                ...
                # （本变体已取消规则②）bh==3 收盘通过 → 成本线上移（第4K起生效）
            else:
...
full.to_csv("results/全波段明细_订单状态机_无保护平仓.csv", index=False, encoding="utf-8-sig")
trade_df.to_csv("results/成交明细_订单状态机_无保护平仓.csv", index=False, encoding="utf-8-sig")
