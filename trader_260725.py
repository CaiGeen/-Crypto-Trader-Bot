...
def _partial_resize_owner_ok(b, owner_op_id):
    """🔥 v6.4-P0：G1/G2/G3b 极窄 owner exception 判据。

    仅当批次处于 partial_resize_pending（净账本已 durable、只差保护单 resize）
    且调用方持有匹配的 close_op_id 时，允许在 close_phase>=1 冻结期创建/替换
    保护单（= partial 事务自身的 resize）。默认 gate 行为零变化，严禁泛化成
    allow_during_close——否则等于重新打开 close 竞态封印。"""
    return (owner_op_id is not None
            and b.get('close_reason') == 'partial_resize_pending'
...
class CryptoTrader:
    ...
    def _start_monitoring(self, symbol: str, batch_id: str, entry_orders: list, stop_steps: list,
        ...
        # ================================================================
            ...
            if isinstance(_fin_b, dict) and _fin_b.get('is_active'):
                try:
                    positions = self._safe_api_call(self.exchange.fetch_positions, [symbol])
                    current_pos = 0.0
                    for pos in positions:
