...
# ⚠️ 第二轮审查（2026-08-21 17:2x）：update_batch_tp 清 invalid flag(+3) + F2 SL/TP fallback reason(+11) → 6 处再 +14
#    重新实测：补挂SL→4511 | 降级恢复→4668 | 补挂TP→4840 | 预生成SL→5143 | 预生成SL兜底→5290 | 预生成TP→5442
# ⚠️ GLM 审计修复（2026-08-21 19:3x）：漏洞4(+3) + 漏洞3(+12) + 漏洞4(+6) → 6 处再 +21
#    重新实测：补挂SL→4534 | 降级恢复→4691 | 补挂TP→4863 | 预生成SL→5166 | 预生成SL兜底→5313 | 预生成TP→5465
# GATE_LINES = {4534, 4691, 4863, 5166, 5313, 5465}  # GLM审计修复后重新 Grep 实测（2026-08-21，AST 函数归属核实）
# ⚠️ D-006 账户层风控（2026-08-28）插码 +116 后未同步（HEAD 实测零交集，T9 陈旧 FAIL 基线）
