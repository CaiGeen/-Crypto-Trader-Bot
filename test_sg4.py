...
# ⚠️ F1/F2/F3（2026-08-21，事件4 TP死锁修复）再偏移：F3 helper(+48) 插在 _order_matches_intent 后
#      （2273 之前+0，之后 +~170：F3 helper + F2 SL/TP terminal 分支 + F1b SL 段 + F1c TP 段）。
#      重新实测 2026-08-21：
#      实测 create 调用点 = [1322, 1473, 1687, 2273, 3817, 3891, 4497, 4654, 4826, 5129, 5276, 5428, 5697, 5880]
# ⚠️ 第二轮审查（2026-08-21 17:2x）：update_batch_tp 清 invalid flag(+3) + F2 SL/TP fallback reason(+11) → +3/+14
#      重新实测：create = [1322, 1476, 1690, 2276, 3820, 3894, 4511, 4668, 4840, 5143, 5290, 5442, 5711, 5894]
# ⚠️ GLM 审计修复（2026-08-21 19:3x）：漏洞4 update_batch_sl 0值(+3) + 漏洞3 孤儿单Fail-Closed(+12) +
#      漏洞4 execute_signal 0值(+6) + 漏洞1 close_position_market 重排(+净变) + 漏洞1b 限价平仓回滚(+净变)
#      重新实测：create = [1322, 1479, 1693, 2299, 3843, 3917, 4534, 4691, 4863, 5166, 5313, 5465, 5720, 5936]
# ⚠️ D-006 账户层风控（2026-08-28）：execute_signal 前插 _check_account_risk helper 三件套（L437 区 +100）
#      + execute_signal 闸门分支(+16) → L2165 后全部 +116。AST 重新实测 2026-08-28：
#      create = [1422, 1579, 1793, 2415, 3959, 4033, 4650, 4807, 4979, 5282, 5429, 5581, 5836, 6052]
...
# ⚠️ P0 Batch B（2026-08-29 深夜）：proof 门重写（L1338 区 +41）+ 调用点插码（+5~8/处）+
#      converge helper 块（L6870 区 +330，仅影响其后 2 处）→ AST 重新实测（同函数纯插码，
#      与上一基线一一对应）：create = [1936, 2099, 2319, 2942, 4826, 4906, 5546, 5709,
#      5887, 6208, 6368, 6533, 6798, 7363]
# ⚠️ D-009 P0（2026-08-29）：全部插码点在最早调用点（L1936）之前——_fsync_dir(L94 后 +28) /
#      load_all_states+_persist_states(L1268 区) / 构造属性(L228 区) / save_batch_state DEGRADED
#      闸门(L1304 区) / 墓碑读写(L1406 区) / _run_position_census + recover 损坏分支(L1653 区)
#      → 14/14 **统一 +222**（AST 实测，位移集合为单值，同函数纯插码）。
A_LINES = {2158, 2321, 2541, 5048, 5128, 5768, 5931, 6109, 6430, 6590, 6755}  # 11 处保护单
B_LINES = {3164}                                                              # 1 处开仓条件单
C_LINES = {7020, 7585}                                                        # 2 处平仓单
