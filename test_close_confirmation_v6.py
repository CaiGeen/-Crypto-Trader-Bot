# -*- coding: utf-8 -*-
"""test_close_confirmation_v6.py — v6 claimed snapshot 绑定 / 市价事务顺序 / ENTRY OrderNotFound 收紧
...
V6_PATH = os.environ.get('V6_HELPER_OVERRIDE') or os.path.join(ASSETS, 'new_helpers_v6.py')
...
def run_txn(impl, block, order_seq, oo_seq=None, pos_seq=None, tb=None,
    ...
    tb = tb if tb is not None else {
        ...
        'target_amounts': [0.001, 0.001], 'filled_details': [77000.0],
...
def main():
    ...
    entry_snapshot = {'last_filled_count': 1, 'target_amounts': [0.001, 0.001],
                      'filled_details': [77000.0], 'side': 'BUY',
    ...
    snap6c = {'last_filled_count': 1, 'target_amounts': [0.001, 0.001],
              'filled_details': [77000.0], 'total_entry_fee': 0.01, 'side': 'BUY'}
    ...
    # BEGIN claim → derive 放行 → gate 的 pending_ids = entry_orders[2:] = []
    # → 无单可撤、恒 True，且不得产生任何 cancel 调用。
    ...
    gate6g = IMPL['_cancel_and_verify_entry_orders'](
        sf6g, SYM, 'batch_A', vars6g, vars6g['last_filled_count']) if ok6g2 else None
    ...
    cases.append(('D6gb 链成功后 gate 零撤单（pending_ids 恒空；含前置条件防假绿）',
                  (ok6g2, gate6g, cancels6g), (True, True, []), None, None,
...
IMPL = extract_impl(V6_PATH, WANT_FUNCS)
